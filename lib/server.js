...
const balanceRouter = require('../route/balance')

class Server {
  constructor (config, logger, database, api) {
    this.config = config
    this.api = api
  ...
  listen (cb) {
    this.logger.verbose('Attempting to bind HTTP server to :%s', this.config.server.port)
  ...
  _setupExpressRoutes () {
    ...
    this.expressApp.use('/balance', balanceRouter)
