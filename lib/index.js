  ...
  constructor (config, logger) {
    ...
    if (config.eth.enabled) {
      ...
      this.tokenExporter = new TokenExporter({
        config: config.eth, logger: this.logger, db: this.database, ethApi: this.ethApi, socket: this.socket
      })
      this.auctionStatus = new AuctionStatus({
        logger: this.logger, db: this.database, ethApi: this.ethApi, socket: this.socket
      })
    }
  }
