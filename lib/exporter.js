...
class Exporter {
    ...
    this.newEvents
      .on('data', (event) => {
        ...
        if (event.event === 'Transfer') {
          this._exportBalance(event.args._from)
          this._exportBalance(event.args._to)
        }

        if (event.event === 'Approval') {
          this._exportBalance(event.args._owner)
          this._exportBalance(event.args._spender)
        }
      })
      .on('changed', (event) => {
  ...
  _exportBalance (address, cb) {
    ...
    this.contract.balanceOf(address, (err, balance) => {
      if (err) {
        return this.logger.error(`Error exporting balance events: ${err}`)
      }
      const event = { _id: address, balance: balance.toNumber() }
