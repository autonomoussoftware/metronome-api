...
class Exporter {
  constructor (config, logger, db) {
    ...
    this.contract = new this.web3.eth.Contract(config.erc20ABI)

    this.contract.options.address = config.tokenAddress
    
    // TODO: validate if this still earing the same events when reach the latest block and the events are duplicated
    this.allEvents = this.contract.getPastEvents({fromBlock: config.exportStartBlock, toBlock: 'latest'})
    // this.newEvents = this.contract.events.allEvents()

    // this.newEvents.on('data').then(log => {
    //   if (err) { return this.logger.info(`Error receiving new log: {err}`) }
    ...
    .catch(err => this.logger.info(`Error receiving historical events: ${err}`))
