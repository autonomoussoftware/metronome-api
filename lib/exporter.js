...
var exporter = function(config, db) {
  ...
  self.web3 = new Web3();
  const provider = new self.web3.providers.HttpProvider('http://localhost:8545')
  ...
  self.processLog = function(log, callback) {
    ...
    console.log("Exporting log:", e._id);
    ...
    self.web3.eth.getBlock(log.blockNumber, false, function(err, block) {
      ...
      self.db.model('Event').create(e, function(err, newLogs) {
        if (err) {
  ...
  self.exportBalance = function(address, callback) {
    ...
    self.contract.balanceOf(address, function(err, balance) {
      ...
      self.db.model('Event').update({ _id: doc._id }, doc, { upsert: true }, function(err, numReplaced) {
        if (err) {
