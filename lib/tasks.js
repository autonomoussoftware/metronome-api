...
class Tasks {
  ...
  _setupAuctionStatusTask () {
    const interval = setInterval(
      () => this.getAuctionStatus().then(res => this.socket.emit(events.AUCTION_STATUS_TASK, res)),
      10000
    )
    ...
    return Promise.all(stats)
      .then(
        ([
          ...
          this.logger.verbose(`Auction status fetched, current price: ${res.currentPrice}`)
          return res
        }
      )
      .catch(err => {
