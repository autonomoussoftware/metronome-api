...
class Metronome {
  ...
  // https://github.com/INFURA/infura/issues/43#issuecomment-350521106
  // This issue is not specific to infura, it can happen with pool of nodes.
  // This recursive function make sure that we never run into issue
  // where 'getBlock()' returns null.
  getTimestamp(blockNumber, cb) {
    this.web3.eth.getBlock(blockNumber, false, (err, block) => {
      if (err || !block) {
        return this.getTimestamp(blockNumber, cb)
        // eslint-disable-next-line no-else-return
      } else {
        return cb(block.timestamp)
      }
    })
  }
  ...
  processEvent(event, cb) {
    ...
    this.getTimestamp(event.blockNumber, timestamp => {
      newEvent.metaData.timestamp = timestamp
